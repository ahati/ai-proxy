...
func TestErrorHandling_MalformedJSON(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			_, err := TransformResponsesToAnthropicWithOptions([]byte(tt.input), nil, true)
			if err == nil {
				t.Error("TransformResponsesToAnthropicWithOptions: expected error for malformed JSON, got nil")
			} else if !strings.Contains(strings.ToLower(err.Error()), tt.expectedErrMsg) {
				t.Errorf("TransformResponsesToAnthropicWithOptions: expected error containing '%s', got: %v", tt.expectedErrMsg, err)
			}
