	...
	tests := []struct {
		...
		},
		{
			...
			validate: func(t *testing.T, output string) {
				...
				// Note: text between GLM5 tool calls (" and ") is emitted as EventContent by the parser
				// but with the chain pattern it arrives after the thinking block is closed, so it
				// is not included in reasoning output.
			},
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			cfg := transform.Config{
				...
				GLM5ToolCallTransform: true,
			}
			transformer, err := buildAnthropicToResponses(&buf, cfg)
			if err != nil {
				t.Fatalf("buildAnthropicToResponses error: %v", err)
			}
